...
def test_basic_hierarchy(app):
    ...
    assert "sample" in content.lower() or "overview" in content.lower()


@pytest.mark.sphinx("html", testroot="basic")
def test_toctree_has_all_docs(app):
    ...
    assert "plans/overview" in includefiles
    assert "plans/2026/03/23/sample" in includefiles
    assert "plans/2026/03/24/another" in includefiles
...
def test_bullet_list_structure(app):
    ...
    # "2026" ディレクトリと "overview" ファイルが含まれる
    text = doctree.astext()
    assert "2026" in text
